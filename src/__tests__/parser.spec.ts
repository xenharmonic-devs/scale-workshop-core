...
describe('Line parser', () => {
  ...
  it('can assign quoted variables', () => {
    const context = new Map();
    const ast = parseAST('"foo bar*" = 13/10');
    const nothing = evaluateAST(ast, context);
    expect(nothing).toBe(undefined);
    expect(context.has('###foo bar*')).toBe(true);
    expectFraction(context.get('###foo bar*')!, '13/10');
  });

  it('can access quoted variables', () => {
    const five = new Quantity(
      ExtendedMonzo.fromFraction(5),
      'scalar',
      new Fraction(0)
    );
    const context = new Map([['###foo bar*', five]]);
    const ast = parseAST('7 % "foo bar*"');
    const quantity = evaluateAST(ast, context);
    expectFraction(quantity!, '7/5');
  });
  ...
  it('cancels mtof with ftom', () => {
    const quantity = parse('ftom(mtof(100))');
    expect(quantity.value.valueOf()).toBeCloseTo(100);
  });

  it('cancels ftom with mtof', () => {
    const quantity = parse('mtof(ftom(123Hz))');
    expectFrequency(quantity, 123);
  });
});
