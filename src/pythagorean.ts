...
// TODO: Swap tonesplitters and semiquartals

// Exponents for neutral interordinal intervals.
const SEMIQUARTAL_VECTORS: number[][] = [
  [4, -2.5],
  [1, -0.5],
  [-2, 1.5],
  [6, -3.5], // [-5, 3.5] would be simpler, but I want the semioctave to be a major interval.
  [3, -1.5],
  [0, 0.5],
  [-3, 2.5],
];
...
export function fromParts(quality: string, degree: number): ExtendedMonzo {
  ...
  let vector: number[];
  if (prototype % 1) {
    vector = [...SEMIQUARTAL_VECTORS[prototype - 0.5]];
  } else {
