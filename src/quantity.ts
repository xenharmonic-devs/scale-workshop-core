import {Fraction, mtof as mtof_} from 'xen-dev-utils';
...
const ILOG_SEMITONE = 1 / Math.log(2 ** (1 / 12));
...
export function ftom(freq: Quantity, baseFrequency?: Quantity) {
  ...
  const index = Math.log(freq.value.valueOf()) * ILOG_SEMITONE + FTOM_OFFSET;
  return new Quantity(ExtendedMonzo.fromValue(index), 'scalar', ZERO);
}
